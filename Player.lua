                ...
                if not DEPUG then
                    if RankIndex(targetSquare) == 0 or RankIndex(targetSquare) == 7 then
                        if Piece().PieceType(Board.Square[targetSquare][1]) == Piece().Pawn then
                            ...
                            Game.promotionColor = ((Piece().IsColor(Board.Square[targetSquare][1], Piece().White)) and
...
function MakeComputerMove(startSquare, targetSquare)
    selectedPieceSquare = startSquare
    if Piece().IsColor(Game.Board.Square[startSquare][1], Piece().Black) then
        Game.plyCount = Game.plyCount + 1
    end
    if Board.Square[targetSquare][1] ~= 0 then
    ...
    -- Handling En Passant
    if Piece().PieceType(Board.Square[startSquare][1]) == Piece().Pawn and
        (RankIndex(targetSquare) - RankIndex(startSquare) == 2 or RankIndex(targetSquare) - RankIndex(startSquare) == -2) then
        Game.epFile = FileIndex(targetSquare) + 1
    else
        Game.epFile = 0
    end
    ...
    if not DEPUG then
        if RankIndex(targetSquare) == 0 or RankIndex(targetSquare) == 7 then
            if Piece().PieceType(Board.Square[targetSquare][1]) == Piece().Pawn then
                Game.promotionAvalible = true
                Game.promotionColor =
                    ((Piece().IsColor(Board.Square[targetSquare][1], Piece().White)) and Piece().Black) or Piece().White
                Game.promotionSquare = targetSquare
            end
        end
    end
